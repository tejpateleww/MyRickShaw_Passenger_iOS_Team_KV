...
let header: [String:String] = ["key":"MyRickshaw123*#*"]
...
func postDataWithoutLoader(_ dictParams: AnyObject, nsURL: String, completion: @escaping (_ result: AnyObject, _ sucess: Bool) -> Void)
{
    let url = WebserviceURLs.kBaseURL + nsURL
...
func getDataWithOrWithoutLoading(_ dictParams: AnyObject, nsURL: String, isLoading: Bool ,completion: @escaping (_ result: AnyObject, _ success: Bool) -> Void)
{
    let url = WebserviceURLs.kBaseURL + nsURL
...
func postDataWithData(_ dictParams: AnyObject, nsURL: String, completion: @escaping (_ data: Data, _ result: AnyObject, _ sucess: Bool) -> Void)
{
    let url = WebserviceURLs.kBaseURL + nsURL
    ...
    Alamofire.request(url, method: .post, parameters: dictParams as? [String : AnyObject], encoding: URLEncoding.default, headers: header)
        ...
        { (response) in
            if let JSON = response.result.value
            {
                if (JSON as AnyObject).object(forKey:("status")) as! Bool == false
                {
                    completion(response.data!, JSON as AnyObject, false)
                    UtilityClass.hideACProgressHUD()
                }
                else
                {
                    completion(response.data!, JSON as AnyObject, true)
                    UtilityClass.hideACProgressHUD()
                }
            }
            else {
