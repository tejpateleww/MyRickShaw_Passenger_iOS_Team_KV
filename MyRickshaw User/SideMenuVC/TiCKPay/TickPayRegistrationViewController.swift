...
//        CardNo,Cvv,Expiry,Alias,CompanyName,ABN(optional),Image
        ...
        if(txtCompanysName.text?.count == 0)
        {
            ...
            UtilityClass.setCustomAlert(title: appName, message: "Please Insert Company Name") { (index, title) in
            }
            return false
        }
        //iconCamera
        ...
        {
            UtilityClass.setCustomAlert(title: appName, message: "Please Upload Driving Licence Image") { (index, title) in
            }
            return false
        }
        else if(btnimgCameraForPassport.imageView?.image == nil && btnimgCameraForPassport.imageView?.image == UIImage(named: "iconCamera"))
        {
            UtilityClass.setCustomAlert(title: appName, message: "Please Upload Passport Image") { (index, title) in
            }
            return false
        }
