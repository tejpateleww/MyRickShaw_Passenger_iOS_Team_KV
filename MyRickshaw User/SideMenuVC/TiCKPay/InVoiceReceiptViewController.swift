...
//            webserviceOFSendInvoice()
    ...
    func validations() -> Bool
    {
        ...
        if txtCustomerName.text!.count == 0 {
            ...
            UtilityClass.setCustomAlert(title: appName, message: "Please enter customer name") { (index, title) in
            }
            return false
        }
        else if txtDescription.text!.count == 0 {

            UtilityClass.setCustomAlert(title: appName, message: "Please enter description") { (index, title) in
            }
            return false
        }
...
//        TickpayId,InvoiceType,CustomerName,Notes,Amount,Email,MobileNo(InvoiceType : SMS/Email)
        ...
        if strInvoiceType == "" {
            UtilityClass.setCustomAlert(title: appName, message: "Select Invoice Type") { (index, title) in
            }
        }
        else {
