...
//        self.aryData = SingletonClass.sharedInstance.aryOnGoing as! NSMutableArray
...
//                 Post notification
    ...
    func webserviceOfOnGoingPagination(index: Int) {
        ...
        webserviceForOnGoingBooking(driverId as AnyObject, isLoading: false) { (result, status) in
            if (status) {
